        ...
        <div class="container">
            <div class="wrap">
                <div class="card">
                    ...
                    <div class="top">
                        <div>
                            <h2 class="custom-section-title" style="margin-bottom: 10px; text-transform: uppercase; font-size: 24px;">Cart Items</h2>
                        </div>
                            ...
                            <tbody id="cartBody">
                                @forelse($cartContents as $key => $item)
                                    @php
                                        ...
                                        // title
                                        $title = $item['plan_name'] ?? $item['name'] ?? ($item['service_title'] ?? 'Untitled');
                                    ...
                                    <tr data-id="{{ $key }}">
                                        ...
                                        <td>
                                            <div class="qty" data-id="{{ $key }}">
                                                ...
                                                <form action="{{ route('cart.updateQty', $key) }}" method="POST" style="display:flex;gap:8px;align-items:center;">
                                                    @csrf
                                                    <input type="number" min="1" name="quantity" value="{{ $qty }}" class="qty-input" style="width:80px;padding:8px;border-radius:8px;border:1px solid #e6e6e6;text-align:center;">
                                                    <button type="submit" style="padding:8px 10px;border-radius:8px;background:#111;color:#fff;border:0;cursor:pointer;font-weight:700">Update</button>
                                                </form>
                                            </div>
                                        </td>
                                        ...
                                        <td>
                                            <form action="{{ route('cart.remove', $key) }}" method="POST" onsubmit="return confirm('Remove this item?');">
                                                @csrf
                                                <button type="submit" class="remove" style="background:transparent;border:0;color:#ef4444;cursor:pointer;font-weight:600">Remove</button>
                                            </form>
                                        </td>
                                    </tr>
                                @empty
